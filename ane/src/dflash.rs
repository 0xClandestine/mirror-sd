...
pub fn build_fused_attn_layer_kernel(w_sq: usize, w_ctx: usize) -> Graph {
    ...
    let k_4d = g
        .reshape(k_concat, Shape { batch: 1, channels: N_KV_HEADS, height: HEAD_DIM, width: w_kv });
    ...
    let v_4d = g
        .reshape(v_concat, Shape { batch: 1, channels: N_KV_HEADS, height: HEAD_DIM, width: w_kv });
...
pub fn build_sdpa_no_gqa_kernel(w_sq: usize, w_kv: usize) -> Graph {
    ...
    let q = g.placeholder(Shape { batch: 1, channels: N_HEADS, height: w_sq, width: HEAD_DIM });
    let k = g.placeholder(Shape { batch: 1, channels: N_HEADS, height: w_kv, width: HEAD_DIM });
    let v = g.placeholder(Shape { batch: 1, channels: N_HEADS, height: w_kv, width: HEAD_DIM });
...
pub fn build_kv_concat_kernel(w_ctx: usize, w_sq: usize) -> Graph {
    let mut g = Graph::new();
    let ctx =
        g.placeholder(Shape { batch: 1, channels: N_KV_HEADS * HEAD_DIM, height: 1, width: w_ctx });
    let noise =
        g.placeholder(Shape { batch: 1, channels: N_KV_HEADS * HEAD_DIM, height: 1, width: w_sq });
...
pub fn build_k_proj_ctx_kernel(w_ctx: usize) -> Graph {
    let mut g = Graph::new();
    let target_hid = g.placeholder(Shape { batch: 1, channels: HIDDEN, height: 1, width: w_ctx });
    let wk = g.placeholder(Shape {
        batch: 1,
        channels: HIDDEN,
        height: 1,
...
pub fn build_rope_q_kernel(w_sq: usize) -> Graph {
    let mut g = Graph::new();
    let q4 = g.placeholder(Shape { batch: 1, channels: N_HEADS, height: w_sq, width: HEAD_DIM });
    let cos_q = g.placeholder(Shape { batch: 1, channels: 1, height: w_sq, width: HEAD_DIM });
    let sin_q = g.placeholder(Shape { batch: 1, channels: 1, height: w_sq, width: HEAD_DIM });
